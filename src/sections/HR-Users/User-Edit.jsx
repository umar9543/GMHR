...
export default function UserEditForm({ currentUser }) {
  ...
  const [tempExService, setTempExService] = useState({
    organization: null, yearOfService: '', reasonOfDischarge: '',
  ...
  const defaultValues = useMemo(() => ({
    // Step 1
    machineCode: currentUser?._raw?.empCode || '',
    fullName: currentUser?.EmployeeName || '',
    fathersName: currentUser?.FatherName || '',
    mothersName: currentUser?.MotherName || '',
    religion: currentUser?._raw?.religion || '',
    nationality: currentUser?.Nationality || '',
    nid: currentUser?.CNIC || '',
    cnicIssueDate: currentUser?.cnicIssueDate || (currentUser?._raw?.cnicIssueDate ? parseDate(currentUser._raw.cnicIssueDate) : null),
    nidExpiryDate: currentUser?.nidExpiryDate || (currentUser?._raw?.cnicExpDate ? parseDate(currentUser._raw.cnicExpDate) : null),
    dateOfBirth: currentUser?.dateOfBirth || (currentUser?._raw?.dob ? parseDate(currentUser._raw.dob) : null),
    age: currentUser?.Age || '',
    maritalStatus: null,
    gender: null,
    department: currentUser?._raw?.depId ? { DepId: currentUser._raw.depId, DepartmentName: currentUser._raw.departmentName || '' } : null,
    section: currentUser?._raw?.sectionID ? { SectionID: currentUser._raw.sectionID, SectionName: currentUser._raw.sectionName || '' } : null,
    designation: currentUser?.Designation || '',
    placeOfBirth: currentUser?.PlaceOfBirth || '',
    cast: currentUser?.Cast ? { id: currentUser.Cast, name: currentUser.Cast } : null,
    noOfDependent: currentUser?._raw?.noOfDependent || '',
    religiousAffiliation: currentUser?._raw?.religiousAffiliation || '',
    politicalAffiliation: currentUser?._raw?.politicalAffiliation || '',
    noOfWife: currentUser?._raw?.noOfWife || '',
    noOfChildren: currentUser?._raw?.noOfChild || '',
    fixedSalary: currentUser?.CurrentSalary || '',
    employeeCategory: currentUser?._raw?.locationPlaceID ? { id: currentUser._raw.locationPlaceID, name: currentUser._raw.locationPlace || '' } : null,
    hiringDate: currentUser?._raw?.empStartDate ? parseDate(currentUser._raw.empStartDate) : null,
    accountNo: currentUser?.BankAccount || '',
    bankBranch: currentUser?.BankBranch || '',
    email: currentUser?._raw?.email || '',
    contactNo: currentUser?._raw?.contactNo || '',
    bloodGroup: null,
    gradeNo: null,
    jobLocation: null,
    photo: null,
    ...
    domicileStatus: currentUser?._raw?.domicileStatus === 1 ? 'Yes' : 'No',
    domicileNo: currentUser?._raw?.domicileNo || '',
    domicileDistrict: currentUser?._raw?.residenceDistrict || '',
    domicileIssueDate: currentUser?._raw?.domicileIssuedate ? parseDate(currentUser._raw.domicileIssuedate) : null,
    passportStatus: currentUser?._raw?.passport === 1 ? 'Yes' : 'No',
    passportNo: currentUser?._raw?.passportNO || '',
    passportExpiryDate: currentUser?._raw?.passportExpDate ? parseDate(currentUser._raw.passportExpDate) : null,
    drivingLicenseStatus: currentUser?._raw?.drivingLicenseStatus === 1 ? 'Yes' : 'No',
    drivingLicenseNo: currentUser?._raw?.drivingLicenceNo || '',
    issuedAuthority: currentUser?._raw?.issueAuthority || '',
    drivingLicenseExpiry: currentUser?._raw?.drivingLicenceExp ? parseDate(currentUser._raw.drivingLicenceExp) : null,
    residenceStatus: currentUser?._raw?.residenceStatusID ? { id: currentUser._raw.residenceStatusID, name: currentUser._raw.residenceStatus || '' } : null,
    residenceDuration: currentUser?._raw?.residenceDuration || '',
    city: currentUser?._raw?.cityID ? { id: currentUser._raw.cityID, name: currentUser._raw.cityName || '' } : null,
    presentAddress: currentUser?.PresentAddress || '',
    fslAccommodation: currentUser?._raw?.fslAccStatus === 1 ? 'Yes' : 'No',
    station: currentUser?._raw?.fslStationid ? { id: currentUser._raw.fslStationid, name: currentUser._raw.fslStation || '' } : null,
    town: currentUser?._raw?.townID ? { id: currentUser._raw.townID, name: currentUser._raw.townName || '' } : null,
    policeStation: currentUser?._raw?.policeStationID ? { id: currentUser._raw.policeStationID, name: currentUser._raw.policeStationName || '' } : null,
    permanentStatus: null,
    permanentDuration: currentUser?._raw?.premanentAddressDuration || '',
    permanentAddress: currentUser?.PermanentAddress || '',
    province: currentUser?._raw?.provinceID ? { id: currentUser._raw.provinceID, name: currentUser._raw.provinceName || '' } : null,
    permanentDistrict: currentUser?._raw?.premanentAddressDistrict || '',
    permanentPoliceStation: currentUser?._raw?.premanentAddressPoliceStation || '',
    mobileNo: currentUser?.MobileNo || '',
    homeNo: currentUser?.HomeNo || '',
    familyNo: currentUser?.FamilyNo || '',
    nextToKin: currentUser?._raw?.nexttokin || '',
    relationship: currentUser?._raw?.relationship || '',
    cnic: currentUser?._raw?.contactCNIC || '',

    // Step 3 - Education & Health
    educationLevel: currentUser?._raw?.educationID ? { id: currentUser._raw.educationID, name: currentUser._raw.education || '' } : null,
    educationInstitute: null,
    educationYear: currentUser?._raw?.educationYear || '',
    motherTongue: currentUser?._raw?.language || '',
    skills: currentUser?._raw?.skills || '',
    hasSports: currentUser?._raw?.sportsStatus === 1 ? 'Yes' : 'No',
    sportsName: currentUser?._raw?.sports || '',
    sportsType: currentUser?._raw?.levelID ? { id: currentUser._raw.levelID, name: currentUser._raw.sportsLevel || '' } : null,
    sportsLevel: currentUser?._raw?.sportsLevel || '',
    prevApsaaCert: currentUser?._raw?.apssaCertify || '',
    prevTraining: currentUser?._raw?.firingPractise || '',
    appearance: currentUser?._raw?.appearenceID ? { id: currentUser._raw.appearenceID, name: currentUser._raw.appearanceLook || '' } : null,
    fitness: currentUser?._raw?.fitnessID ? { id: currentUser._raw.fitnessID, name: currentUser._raw.fitnessLevel || '' } : null,
    bearing: currentUser?._raw?.bearingID ? { id: currentUser._raw.bearingID, name: currentUser._raw.bearingLevel || '' } : null,
    height1: currentUser?._raw?.heigthFtId ? { id: currentUser._raw.heigthFtId, name: String(currentUser._raw.heightFt || '') } : null,
    height2: currentUser?._raw?.heigthInchId ? { id: currentUser._raw.heigthInchId, name: String(currentUser._raw.heightInch || '') } : null,
    // chest1: currentUser?._raw?.chestId ? { id: currentUser._raw.chestId, name: String(currentUser._raw.chestSize || '') } : null,
    chest2: currentUser?._raw?.chestId2 ? { id: currentUser._raw.chestId2, name: String(currentUser._raw.chestSize2 || '') } : null,
    weight: currentUser?._raw?.weightId ? { id: currentUser._raw.weightId, name: String(currentUser._raw.weight || '') } : null,
    color: currentUser?._raw?.colorId ? { id: currentUser._raw.colorId, name: currentUser._raw.color || '' } : null,
    healthBloodGroup: currentUser?._raw?.bloodGroupid ? { id: currentUser._raw.bloodGroupid, name: currentUser._raw.bloodGroup || '' } : null,
    eyeSight: currentUser?._raw?.eyesightId ? { id: currentUser._raw.eyesightId, name: currentUser._raw.eyeSight || '' } : null,
    criticalDiagnosis: currentUser?._raw?.criticalDiagnoseID ? { id: currentUser._raw.criticalDiagnoseID, name: currentUser._raw.criticalDiagnose || '' } : null,
    hospitalization: currentUser?._raw?.hospitalization || '',
    medicalScreening: currentUser?._raw?.medScreenID ? { id: currentUser._raw.medScreenID, name: currentUser._raw.medScreen || '' } : null,
    hasMajorSurgery: currentUser?._raw?.majorSurjeryStatus === 1 ? 'Yes' : 'No',
    surgeriesList: (currentUser?.Surgeries || currentUser?._raw?.surgeries || []).map((s) => ({
      surgeryDetails: s.majorSurery || '',
      surgeryYear: s.surjeryYear || ''
    })),
    dailyMedicine: currentUser?._raw?.dailyMedicine || '',
    habit: currentUser?._raw?.habitID ? { id: currentUser._raw.habitID, name: currentUser._raw.habit || '' } : null,

    // Step 4: Experience
    civilianExperiences: (currentUser?.CivilExperiences || currentUser?._raw?.civilExperiences || []).map((e) => ({
      organization: e.cvOrganization || '',
      yearOfDuration: e.cvExDuration || '',
      reasonOfLeaving: e.reasonOfleavingCv || ''
    })),
    exServiceExperiences: (currentUser?.UniformExperiences || currentUser?._raw?.uniformExperiences || []).map((e) => ({
      organization: { id: e.uniformOrganizationId || 0, name: e.uniformOrganization || '' },
      yearOfService: e.unyearofService || '',
      reasonOfDischarge: e.uniReasonofDischarge || '',
      designation: e.designation || '',
      dateOfDischarge: e.dateofDischarge ? parseDate(e.dateofDischarge) : null,
      serviceBook: e.serviceBookStatus || 'No'
    })),

    // Step 5: Guarantor & Reference
    g1Name: currentUser?._raw?.guarantor1Name || '',
    g1FatherName: currentUser?._raw?.guarantor1FName || '',
    g1Cnic: currentUser?._raw?.guarantor1Cnic || '',
    g1Address: currentUser?._raw?.guarantor1Address || '',
    g1NadraVerify: currentUser?._raw?.nadraVerGr1Status === 1 ? 'Yes' : 'No',
    g1CellNo: currentUser?._raw?.guarantor1CellNo || '',
    g1VerifyStatus: currentUser?._raw?.guarantor1VerifyStatus ? { id: currentUser._raw.guarantor1VerifyStatus, name: currentUser._raw.guarantor1VerifyStatusText || '' } : null,
    g1VerifiedBy: currentUser?._raw?.g1VerifiedBy || '',
    g1VerifiedOn: currentUser?._raw?.g1VerifiedOn ? parseDate(currentUser._raw.g1VerifiedOn) : null,

    g2Name: currentUser?._raw?.guarantor2Name || '',
    g2FatherName: currentUser?._raw?.guarantor2FName || '',
    g2Cnic: currentUser?._raw?.guarantor2Cnic || '',
    g2Address: currentUser?._raw?.guarantor2Address || '',
    g2NadraVerify: currentUser?._raw?.nadraVerGr2Status === 1 ? 'Yes' : 'No',
    g2CellNo: currentUser?._raw?.guarantor2CellNo || '',
    g2VerifyStatus: currentUser?._raw?.guarantor2VerifyStatus ? { id: currentUser._raw.guarantor2VerifyStatus, name: currentUser._raw.guarantor2VerifyStatusText || '' } : null,
    g2VerifiedBy: currentUser?._raw?.g2VerifiedBy || '',
    g2VerifiedOn: currentUser?._raw?.g2VerifiedOn ? parseDate(currentUser._raw.g2VerifiedOn) : null,

    refName: currentUser?._raw?.referenceName || '',
    refFatherName: currentUser?._raw?.refFName || '',
    refCnic: currentUser?._raw?.refNic || '',
    refAddress: currentUser?._raw?.refAddress || '',
    refVerifyStatus: currentUser?._raw?.refVerifyStatus ? { id: currentUser._raw.refVerifyStatus, name: currentUser._raw.refVerifyStatusText || '' } : null,
    refCellNo: currentUser?._raw?.refCellNo || '',

    // Step 6: Uploads & Additional
    uploadsList: [],
    disciplinePerformance: currentUser?._raw?.disciplane || '',
    howKnowFsl: currentUser?._raw?.knowAbout || '',
    hasRelativeInFsl: currentUser?._raw?.relatedFSLStatus === 1 ? 'Yes' : 'No',
    relativeEmpNo: currentUser?._raw?.fslEmployeeNo || '',
    relativeName: currentUser?._raw?.flsRelationName || '',
    relativeCell: currentUser?._raw?.flsRelationCell || ''
  }), [currentUser]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [FetchDepartmentOptions, FetchSectionOptions, FetchDesignationOptions, FetchBloodGroupOptions, FetchGradeOptions, FetchJobLocationOptions, fetchHrFalconDropdowns]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const payload = {
        hrEmployeeID: Number(currentUser?.HrEmployeeID || currentUser?._raw?.hrEmployeeID || 0),
      ...
      enqueueSnackbar(`Employee updated — Code: ${result.empCode}`, { variant: 'success' });
      ...
      <Box sx={{ p: 2, borderRadius: 1 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid xs={12} md={4} display="flex" alignItems="center" justifyContent="flex-end">
            <Button variant="contained" color="primary" onClick={() => {
              if (tempExService.organization) { appendExServ(tempExService); setTempExService({ organization: null, yearOfService: '', reasonOfDischarge: '', designation: '', dateOfDischarge: null, serviceBook: 'No' }); }
            }}>Add</Button>
          </Grid>
        </Grid>
